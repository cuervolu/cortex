...
#[cfg(target_os = "linux")]
use crate::ollama::check_ollama_linux;

#[cfg(target_os = "macos")]
use crate::ollama::check_ollama_macos;

#[cfg(target_os = "windows")]
use crate::ollama::check_ollama_windows;

use crate::ollama::CHAT_CONTEXTS;
use crate::ollama_models::{force_update_ollama_models, update_ollama_models, OllamaModel};
use crate::{OllamaModelDetails, OllamaModelInfo};
use error::AppError;

#[tauri::command]
pub async fn get_ollama_models(app_handle: AppHandle<Wry>) -> Result<Vec<OllamaModel>, AppError> {
    crate::ollama_models::get_ollama_models(&app_handle).await
}
...
pub async fn send_prompt_to_ollama(
    ...
    let ollama = crate::ollama::get_ollama()?;
