...
import type { RoadmapCreateRequest, RoadmapUpdateRequest } from "@cortex/shared/types";
...
const handleSubmit = async (formData: RoadmapFormValues | CourseModuleFormValues, imagePath: string | null) => {
  ...
  try {
    if (isCreateMode.value) {
      const request = transformFormToRequest<RoadmapCreateRequest>(formData, false);
      const result = await createRoadmap(request, imagePath);
      await router.push(`/admin/roadmaps/${result.slug}`);
    } else if (roadmap.value) {
      const request = transformFormToRequest<RoadmapUpdateRequest>(formData, true);
      await updateRoadmap(roadmap.value.id, request, imagePath);
    }
  } catch (e) {
    await handleError(e, {
      statusCode: 400,
