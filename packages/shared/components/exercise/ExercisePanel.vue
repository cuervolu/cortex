...
<template>
  <div>
  <!-- Panel for desktop -->
  <ScrollArea class="hidden sm:flex flex-col w-full h-full bg-muted/40 shadow-lg rounded-lg">
    <Tabs :default-value="defaultTab" class="w-full h-full flex flex-col">
      <TabsList class="flex items-start px-6 py-2 bg-muted/40  border-b-2 border-purple-600 shrink-0">
        <TabsTrigger
          v-for="tab in tabs"
          :key="tab.value"
          :value="tab.value"
